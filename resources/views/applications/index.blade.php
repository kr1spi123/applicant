...
@section('content')
    <link rel="stylesheet"
        href="{{ asset('css/lkapp.css') . '?v=' . (file_exists(public_path('css/lkapp.css')) ? filemtime(public_path('css/lkapp.css')) : time()) }}">
    <link rel="stylesheet"
        href="{{ asset('css/auth.css') . '?v=' . (file_exists(public_path('css/auth.css')) ? filemtime(public_path('css/auth.css')) : time()) }}">
        ...
        <div class="container" style="display: block; max-width: 1200px;">
            @if(session('success'))
                <div class="success-message active"
                    style="background-color: #E8F5E9; color: #2E7D32; padding: 16px; border-radius: 8px; border: 1px solid #C8E6C9; margin-bottom: 16px;">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div style="display: grid; grid-template-columns: 2.3fr 1.7fr; gap: 24px; margin-bottom: 24px;">
                <div class="application-card" style="margin: 0;">
                    <div class="card-header">
                        <div>
                            <div style="font-size: 14px; color: #9A9CA5; margin-bottom: 4px;">Личный кабинет абитуриента
                            </div>
            ...
            <div class="applications-header">
                <div>
                    ...
                    <p class="applications-subtitle">Отслеживайте статус поданных документов и параметры выбранных программ
                        обучения</p>
                </div>
            </div>
            ...
            @if($applications->count() > 0)
                <div class="applications-list">
                    @foreach($applications as $application)
                        ...
                        <div class="application-card" data-code="{{ $code }}" data-form="{{ mb_strtolower($selectedForm) }}"
                            data-fee="{{ $selectedFee ?? '' }}" data-date="{{ $createdAtTimestamp ?? '' }}">
                            <div class="card-header">
                                ...
                                <div class="application-scores-grid">
                                    <div class="application-score-item">
                                        ...
                                        <span class="application-score-value">
                                            <svg class="icon-sm" style="color: #FF5A30;" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <div class="application-score-item">
                                        ...
                                        <span class="application-score-value">
                                            <svg class="icon-sm" style="color: #4CAF50;" viewBox="0 0 24 24" fill="none"
                                                stroke="currentColor" stroke-width="2" aria-hidden="true">
            ...
            @else
                <div class="no-applications">
                    ...
                    <p style="margin-top: 10px; color: #666;">Подайте заявление на поступление, выбрав интересующую
                        специальность.</p>
                    <a href="{{ route('applications.create') }}"
                        style="display: inline-block; margin-top: 20px; background: #FF5A30; color: white; padding: 10px 24px; border-radius: 8px; text-decoration: none; font-weight: 600;">Подать
                        заявку</a>
                </div>
            @endif
        </div>
