    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (benefitFileInput) {
            benefitFileInput.addEventListener('change', function(e) {
                ...
                if (tooBig.length) {
                    alert('Некоторые файлы превышают 5MB: ' + tooBig.map(function(f) {
                        return f.name;
                    }).join(', '));
                    benefitFileInput.value = '';
                    renderBenefitFiles([]);
                    ...
                    if (!response.ok) {
                        return response.json().then(function(data) {
                            ...
                            throw {
                                status: response.status,
                                data: null,
                                nonJson: contentType.indexOf('application/json') === -1
                            };
                        });
                    }
        ...
        function buildPreviewSection() {
            ...
            if (documentsSection) {
                ...
                const benefitFiles = benefitFileInput && benefitFileInput.files && benefitFileInput.files.length ?
                    Array.from(benefitFileInput.files).map(function(f) {
                        return f.name;
                    }).join(', ') :
                    'Файлы не выбраны';
